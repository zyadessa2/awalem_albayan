...
import { getPublishedBooks } from "@/lib/data/content";
...
function BooksLink() {
  return (
    <Link
      href="/books"
      className="group z-50 flex h-[53px] w-full max-w-[272px] items-center justify-center gap-2 rounded-2xl py-4 text-sm font-bold text-black"
    >
       <span className="whitespace-nowrap text-base underline underline-offset-4 transition group-hover:text-[#c2187a] sm:text-xl">
        جميع الكتب التعليمية
      </span>
...
function BooksDecorations() {
  return (
    <div className="pointer-events-none absolute inset-0 hidden lg:block" aria-hidden>
      <div className="absolute left-[106px] top-[88px] h-[113px] w-[123px] rotate-[30.232deg] bg-[url('/1c8bf21d9d65aaebf8b5ca5971df992a880b0f36.png')] bg-no-repeat opacity-80 [aspect-ratio:123/113] [background-position:-554.503px_-581.767px] [background-size:465.041%_523.894%]" />
      <div className="absolute left-[554px] top-[118px] h-[140px] w-[154px] rounded-[109px] bg-[url('/f680db94eed0fae6abedf3cc045f1894b3a0ecfe.png')] bg-no-repeat opacity-20 [aspect-ratio:11/10] [background-position:-64.058px_-21.724px] [background-size:172.396%_141.379%]" />
    </div>
  );
}

export default async function BooksSection() {
  const publishedBooks = await getPublishedBooks(3);
  const visibleBooks =
    publishedBooks.length > 0
      ? publishedBooks.map((book) => {
          const seriesId =
            typeof book.seriesId === "object" && book.seriesId
              ? book.seriesId._id
      ...
      : books.map((book) => ({
          ...
          href: `/books/1/${book.id + 1}`,
        }));

  return (
    <section className={`relative overflow-hidden bg-white px-4 py-16 sm:px-5 md:py-20 lg:py-24 ${almarai.className}`}>
      <BooksDecorations />
      <div className="pointer-events-none absolute left-20 top-28 hidden -rotate-[18deg] text-7xl font-black tracking-[2px] text-[#ececec] lg:block">
        ABC
      </div>
      <div className="pointer-events-none absolute left-[28%] top-32 hidden text-6xl text-[#f0dcb9] lg:block">
        ✦
      </div>
      ...
      <div className="mx-auto max-w-7xl">
        <div data-reveal="right" className="mb-36 flex flex-col-reverse items-center justify-between gap-8 text-center sm:mb-40 sm:items-start sm:text-right lg:mb-44 lg:flex-row-reverse lg:items-start">
          ...
          <div className="w-full text-center sm:text-right lg:w-[789px] lg:text-right">
            <h2 className="text-4xl font-bold leading-normal text-[#141219] md:text-5xl">
              سلاسل <span className="text-[#c2187a]">الكتب</span>
            </h2>
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-24 md:grid-cols-2 lg:gap-y-24 xl:grid-cols-3 xl:gap-x-8">
